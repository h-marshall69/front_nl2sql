...
<template>
  <div class="min-h-screen">
    <header class="flex items-center justify-between px-5 py-4 border-b border-border-subtle backdrop-blur-md sticky top-0 bg-app-bg/65 z-10">
    ...
    <main class="grid grid-cols-1 lg:grid-cols-[280px_1fr_320px] gap-4 p-4">
      ...
      <aside class="flex flex-col gap-3 bg-panel border border-border-subtle rounded-2xl p-4">
        <div class="flex items-center justify-between">
          ...
          <button class="text-xs hover:text-white transition-colors" @click="refreshTables" :disabled="busy">Actualizar</button>
        </div>
        ...
        <div class="flex flex-col gap-2">
          <button
            ...
            :class="selectedTable === t ? 'ring-2 ring-indigo-500/50 bg-white/10' : 'bg-white/5 hover:bg-panel-hover'"
          >
        ...
        <div v-if="selectedTable" class="mt-4 pt-4 border-t border-white/5">
          <h3 class="text-sm font-bold mb-3">Esquema: <span class="font-mono text-indigo-400">{{ selectedTable }}</span></h3>
          <div v-if="describeLoading" class="text-xs text-white/40 animate-pulse">Cargando…</div>
          <div v-else class="space-y-1">
            <div v-for="c in tableSchema" :key="c.cid" class="grid grid-cols-[1fr_auto] gap-2 items-center p-2 rounded-lg bg-black/20 text-[11px]">
              <span class="font-mono">{{ c.name }}</span>
              <span class="text-white/40">{{ c.type || 'TEXT' }}</span>
              <div class="flex gap-1 col-span-2">
                <span v-if="c.pk" class="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">PK</span>
                <span v-if="c.notnull" class="px-2 py-0.5 rounded bg-api-warn/20 text-api-warn border border-api-warn/30">NOT NULL</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="flex flex-col gap-4">
        <div class="bg-panel border border-border-subtle rounded-2xl p-4">
            ...
            <div class="flex items-center gap-4">
              <label class="flex items-center gap-2 cursor-pointer group">
                <div class="relative">
                  ...
                  <div class="w-10 h-5 bg-white/10 rounded-full peer peer-checked:bg-api-ok/30 border border-white/20 transition-all"></div>
                  <div class="absolute left-1 top-1 w-3 h-3 bg-white/70 rounded-full transition-all peer-checked:translate-x-5 peer-checked:bg-white"></div>
                </div>
                <span class="text-xs text-white/60 group-hover:text-white transition-colors">allow_write</span>
              </label>

              <button class="btn-base bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 border-indigo-500/30" 
                      @click="runNL" :disabled="busy || !nl.trim()">
                ▶ Ejecutar
              </button>
            </div>
          </div>
          ...
          <textarea
            ...
            placeholder="Ej: crear una tabla llamada usuarios..."
          ></textarea>

          <div class="md:grid-cols-2 gap-4 mt-4">
            <div class="p-4 rounded-xl bg-black/30 border border-white/5">
              <h3 class="text-xs font-bold text-white/50 mb-2 uppercase">SQL Generado</h3>
              <pre class="font-mono text-sm text-emerald-400 overflow-x-auto">{{ lastSQL || '—' }}</pre>
            </div>
          </div>
        </div>
        ...
        <div class="bg-panel border border-border-subtle rounded-2xl p-4 flex-1">
          ...
          <div v-if="error" class="p-3 rounded-xl bg-api-bad/10 border border-api-bad/30 text-api-bad text-sm">
            {{ error }}
          </div>

          <div v-else-if="blocked" class="p-3 rounded-xl bg-api-warn/10 border border-api-warn/30 text-api-warn text-sm">
            Bloqueado: {{ blockedReason }}
          </div>

          <template v-else>
            <div v-if="resultType === 'write'" class="p-4 rounded-xl bg-api-ok/10 border border-api-ok/30 text-api-ok">
              ✅ Ejecutada con éxito. Filas afectadas: <b>{{ rowcount }}</b>
            </div>

            <div v-else-if="resultType === 'select'" class="space-y-2">
              <div class="text-xs text-white/40 italic">Filas encontradas: {{ rowcount }}</div>
              <div class="overflow-x-auto rounded-xl border border-white/5">
                <table class="w-full text-left border-collapse">
                  <thead class="bg-white/5 text-white/60 text-xs">
                    <tr>
                      <th v-for="c in columns" :key="c" class="p-3 font-mono border-b border-white/10">{{ c }}</th>
                    </tr>
                  </thead>
                  <tbody class="text-sm">
                    <tr v-for="(r, idx) in rows" :key="idx" class="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td v-for="c in columns" :key="c" class="p-3 font-mono text-white/80">
                        {{ r[c] }}
                      </td>
                    </tr>
                  </tbody>
      ...
      <aside class="bg-panel border border-border-subtle rounded-2xl p-4 overflow-hidden flex flex-col">
        <div class="flex items-center justify-between mb-4">
          <h2 class="font-bold text-sm uppercase text-white/50">Activity Logs</h2>
          <button class="text-[10px] bg-white/5 px-2 py-1 rounded hover:bg-white/10" @click="logs = []">Limpiar</button>
        </div>

        <div class="space-y-2 overflow-y-auto pr-2 custom-scrollbar">
          <div v-for="(l, i) in logs" :key="i" class="p-3 rounded-lg bg-black/20 border border-white/5 text-[11px]">
...
async function refreshTables() {
  try {
    // Si la API devuelve { "tables": [...] }, listTables() debe retornar ese array
    const res = await listTables()
    // Manejo de seguridad por si viene como objeto o array
...
async function selectTable(t) {
  ...
  try {
    ...
    // Según tu Postman, la respuesta es { "columns": [...] }
    tableSchema.value = d.columns || []
    log(`GET /describe/${t} -> ok`)
  } catch (e) {
    describeError.value = 'Error al cargar esquema'
...
async function runSQL() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || 'Error en SQL'
  } finally {
    busy.value = false
  }
}
...
<style scoped>
/* Importante: Reemplaza './assets/main.css' por la ruta real a tu CSS principal */
@reference "./style.css"; 
...
.custom-scrollbar::-webkit-scrollbar-thumb {
  /* Ahora Tailwind reconocerá estas clases */
